...
class CommentsServices:
    ...
    async def create_comment(self, task_id, author_id, text):
        await self.tasks_service.get_task_check_user_permission_by_task_id(task_id=task_id, user_id=author_id,
                                                                           roles=["member", "owner"])
    ...
    async def get_comments(self, task_id, user_id):
        await self.tasks_service.get_task_check_user_permission_by_task_id(task_id=task_id, user_id=user_id,
                                                                           roles=["member", "owner"])
